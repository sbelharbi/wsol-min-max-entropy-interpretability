...
from stain_tools.tools import check_image_and_squeeze_if_gray

# TODO: Create a reading function using PIL.
...
def read_image(path):
    ...
    return im  # numpy array, uint8, (h, w, 3)
...
def plot_row_colors(C, fig_size=6, title=None):
    ...
    :param C: An array of Nx3 where the rows are considered as RGB colors.
    :param fig_size:
    :param title:
    :return:
    """
    ...
    assert C.shape[1] == 3, "C must have 2 columns."

    N = C.shape[0]
    range255 = C.max() > 1.0  # check if we have an image in range [0, 1] or [0, 255].
...
def plot_image(image, show=True, fig_size=10, title=None):
    """
    Plot an image in [0, 1].
    Caution: rescale image to be in range [0, 1].

    :param image:
    :param show: plt.show now?
    :param fig_size:  Size of the largest dimension.
...
def make_image_stack(images):
    """
    Build a stack of image from a tuple/list of images.
    ...
    for image in images:
        assert image.ndim == images[0].ndim  # check all images have the same number of dimensions.
...
def plot_image_stack(ims, width=5, sub_sample=False, rand=False, save_name=None, title_list=None, show=True):
    ...
    :param ims: A patch stack (from make_image_stack()).
